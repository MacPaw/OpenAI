...
public struct ChatQuery: Equatable, Codable, Streamable {
    ...
    public enum ChatCompletionMessageParam: Codable, Equatable {
        ...
        public var reasoning: String? { get {
            switch self {
                case .assistant(let assistantMessage):
                    if let reasoning = assistantMessage.reasoningContent {
                        return reasoning
                    }
                    return nil
                default:
        ...
        public struct AssistantMessageParam: Codable, Equatable {
            ...
            public init(
                ...
                self.content = content
                // We could set either `openrouterReasoningContent` or `deepseekReasoningContent` here.
                // Doesn't really matter right now, but we might wanna find a better way to handle this.
                self.deepseekReasoningContent = reasoning
                self.openrouterReasoningContent = nil
                self.name = name
